...
rootfs_extract_image() {
    ...
    rsync -a $imagefs/. $ROOTFS/ \
        ...
        --include='/var/usr/' \
        --include='/var/usr/lib/' \
        --include='/var/usr/lib/locale/***' \
        --exclude='/var/*' \
        --exclude /sysroot --exclude /boot --exclude /lib/ostree \
        --exclude /ostree --exclude /root --exclude /srv --exclude /opt \
        --exclude /mnt --exclude /home --exclude /usr/local --exclude /usr/lib/locale \
        && rm -Rf "$imagefs"
}

rootfs_include_image() {
    local include_image="${1:-}"
    if [ -z "$include_image" ]; then 
        return 0
    fi
    ci_group_start "rootfs-include-image"
    skopeo copy \
        --src-daemon-host unix:///var/run/podman.sock \
        docker-daemon:$include_image \
        containers-storage:[overlay@$ROOTFS/var/lib/containers/storage:mount_program=/usr/bin/fuse-overlayfs]$include_image
    ci_group_end
}
...
rootfs_install_livesys() {
    ...
    rm -rf /var/lib/dpkg && ln -s /usr/lib/dpkg /var/lib/dpkg && apt clean && apt update && \
    DEBIAN_FRONTEND=noninteractive apt install -y dracut dracut-live calamares calamares-settings-debian
    KERNELVERSION=$(basename "$(find /usr/lib/modules -mindepth 1 -maxdepth 1 -type d | head -n 1)")
    cp /usr/lib/modules/$KERNELVERSION/vmlinuz /boot/vmlinuz
    dracut --kver "$KERNELVERSION" --add "dmsquash-live" --force /boot/initrd.img'
    chroot_function "$CMD"
...
iso() {
    ...
    mkdir -p "${ISOROOT}/boot" "${ISOROOT}/LiveOS" "${ISOROOT}/isolinux"
    [ -f ${ROOTFS}/boot/vmlinuz ] && mv ${ROOTFS}/boot/vmlinuz ${ISOROOT}/LiveOS/
    [ -f ${ROOTFS}/boot/initrd.img ] && mv ${ROOTFS}/boot/initrd.img ${ISOROOT}/LiveOS/
    [ -f ${WORKDIR}/squashfs.img ] && mv ${WORKDIR}/squashfs.img ${ISOROOT}/LiveOS/
    local kargs=()
...
show_config() {
    ...
    local extra_kargs="${5:-$DEFAULT_EXTRA_KARGS}"
...
build() {
    ...
    local include_image="${4:-$DEFAULT_INCLUDE_IMAGE}"
    local extra_kargs="${5:-$DEFAULT_EXTRA_KARGS}"
    show_config "$image" "$iso_output_file" "$iso_disk_label" "$include_image" "$extra_kargs"
    clean
    trap clean ERR INT TERM EXIT
    init_work
    rootfs_bootstrap
    rootfs_extract_image "$image"
    rootfs_include_image "$include_image"
    rootfs_install_livesys "$HOOK_configure_livesys"
    rootfs_squash
    iso "$iso_output_file" "$iso_disk_label" "$extra_kargs"
}
...
usage() {
    ...
    echo "    [image=$DEFAULT_IMAGE]"
    echo "  include"
    echo "    [image=$DEFAULT_INCLUDE_IMAGE]"
    echo "  install"
    echo "  squash"
    echo "  iso"
    echo "    [iso_output_file=$DEFAULT_ISO_OUTPUT_FILE]"
...
case "$COMMAND" in
    ...
    show-config)
        ...
        ;;
    extract)
        rootfs_extract_image "${1:-}"
        ;;
    include)
        rootfs_include_image "${1:-}"
        ;;
    install)
        rootfs_install_livesys "${1:-}"
        ;;
    squash)
        rootfs_squash "${1:-}"
        ;;
